...
/**
 * cv-sync-check.mjs — Validates that the careerforge setup is consistent.
 *
 * Checks:
 * 1. cv.md exists
 * 2. config/profile.yml exists and has required fields
 * 3. No hardcoded metrics in _shared.md or batch/batch-prompt.md
 * 4. templates and fonts exist
 * 5. article-digest.md freshness (if exists)
 * 6. article-digest.md structure (if schema exists)
 */
...
// 4. Check templates and fonts
const templatePath = join(projectRoot, 'templates', 'cv-template.html');
if (!existsSync(templatePath)) {
  errors.push('templates/cv-template.html not found. PDF generation requires this template.');
}
...
if (!existsSync(fontsDir)) {
  warnings.push('fonts/ directory not found. PDF rendering may fall back to system fonts.');
} else {
  for (const fontFile of requiredFonts) {
    const fontPath = join(fontsDir, fontFile);
    if (!existsSync(fontPath)) {
      warnings.push(`Missing font file: fonts/${fontFile}. PDF rendering may look different.`);
    }
  }
}
...
// 6. Check article-digest.md structure against schema if available
const schemaPath = join(projectRoot, 'schemas', 'article-digest.schema.json');
if (existsSync(digestPath) && existsSync(schemaPath)) {
  try {
    ...
    if (schema.minSections && sections.length < schema.minSections) {
      warnings.push(`article-digest.md has ${sections.length} sections. Expected at least ${schema.minSections}.`);
    }
    ...
    sections.forEach((section, idx) => {
      for (const field of requiredFields) {
        if (!section.includes(field)) {
          warnings.push(`article-digest.md section ${idx + 1} is missing required field: ${field}`);
        }
      }
    });
  } catch (err) {
    warnings.push(`Failed to parse schemas/article-digest.schema.json: ${err.message}`);
  }
}

// Output results
console.log('\n=== careerforge sync check ===\n');
